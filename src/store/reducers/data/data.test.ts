...
import {film, films, favoriteFilm, nonFavoriteFilm} from "../../../test-data/films";
...
describe(`Operations Data`, () => {
  ...
  it(`Should post review to /comments/12`, () => {
    ...
    return postingReview(dispatch, () => noop, api)
          .then(() => {
            expect(dispatch).toHaveBeenCalledWith({
  ...
  it(`Should post favorite film status`, () => {
    ...
    return postingFavoriteStatus(dispatch, () => noop, api)
          .then(() => {
            expect(dispatch).toHaveBeenCalledWith({
